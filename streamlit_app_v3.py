...
def main():
    ...
    with st.expander("📋 利用規約", expanded=False):
        # 利用規約をMarkdownで直接表示（Wordファイルの内容をここに貼り付けてください）
        st.markdown("""
        ### 利用規約
        ...
        **最終更新日：2026年1月31日**
        ...
        #### 第2条（サービスの内容）
        本サービスは、建築図面（PDF/画像）から壁線を抽出し、3Dデータを生成するツールを提供します。
        生成されたデータは、Blenderなどの3Dソフトウェアでの利用が可能です。
        ...
        #### 第4条（免責事項）
        本サービスは、現状有姿で提供されるものであり、明示または黙示を問わず、いかなる保証も行いません。
        本サービスの利用により生じたいかなる損害についても、当方は一切の責任を負いません。
        
        - 生成されたデータの精度や正確性について保証しません
        - 本サービスの継続的な提供を保証しません
        - 本サービスの不具合、エラー、障害について責任を負いません
        - ユーザーがアップロードしたデータの損失について責任を負いません
        
        #### 第5条（データの取り扱い）
        ユーザーがアップロードしたファイルは、処理のために一時的にサーバーに保存されますが、セッション終了後は削除されます。
        ただし、サービス改善のために匿名化されたデータを使用する場合があります。
        
        #### 第6条（著作権）
        ユーザーがアップロードした図面の著作権は、ユーザーまたは元の著作権者に帰属します。
        本サービスを通じて生成されたデータの利用については、元の図面の著作権に従ってください。
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #888; padding: 20px; margin-top: 20px; border-top: 1px solid #ddd;'>
            <p style='margin: 5px 0; font-size: 0.9em;'>© 2026 一条工務店 CAD図面3D化アプリ - All Rights Reserved</p>
