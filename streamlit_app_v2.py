...
def _find_nearest_wall_from_click(click_x, click_y, walls, scale, margin, img_height, min_x, min_y, max_x, max_y, threshold=20):
    ...
    Args:
        click_x, click_y: クリック位置（ピクセル座標）
        walls: 壁のリスト
        scale: メートル→ピクセルのスケール
        margin: 可視化画像のマージン
        img_height: 可視化画像の高さ
        min_x, min_y, max_x, max_y: 壁座標の範囲（メートル）
        threshold: 検出閾値（ピクセル、デフォルト20px）
    
    Returns:
        (wall, distance): 最も近い壁とその距離、見つからない場合は (None, None)
    """
    ...
    for wall in walls:
        ...
        # 点から線分までの距離を計算
        distance = _point_to_line_segment_distance(
            click_x, click_y,
...
def main():
    ...
    with st.expander("Step 3：手動編集", expanded=(st.session_state.workflow_step == 3)):
        ...
        if edit_mode == "線を結合":
            st.markdown(
                "結合したい2本の壁線を1回ずつクリックして選択し、壁線を結合します。\n"
                "壁線を直接クリックするだけで選択できます。複数ペアの結合も可能です。\n"
            )
        elif edit_mode == "窓を追加":
            st.markdown(
                "窓追加したい2本の壁線を1回ずつクリックして選択し、その間に窓を追加します。\n\n"
                "壁線を直接クリックするだけで選択できます。複数の窓の追加も可能です。\n\n"
            ...
            st.markdown(
                "削除したい壁線を1回ずつクリックして選択し、選択した壁線を削除します。\n\n"
                "壁線を直接クリックするだけで選択できます。複数本の削除も可能です。"
            )
        ...
        with st.expander("💡 使い方", expanded=False):
            if edit_mode == "線を結合":
                st.markdown(
                    "**複数線結合の手順:**\n\n"
                    "1. 下の画像上で結合したい**1本目の壁線をクリック**して選択（青色でハイライト）\n\n"
                    "2. **2本目の壁線をクリック**して選択（緑色でハイライト）\n\n"
                    "3. さらに結合したい箇所があれば手順1-2を繰り返す（2本ずつペアで選択）\n\n"
                    "4. 「🔗 結合実行」で選択した全てのペアを一括結合\n\n"
                    "**ヒント:**\n\n"
                    "- 壁線を直接クリックするだけで選択できます\n\n"
                    "- 奇数本目（1本目、3本目...）は青、偶数本目（2本目、4本目...）は緑で表示\n\n"
                    "- 間違えた場合は同じ壁をもう一度クリックで選択解除"
                )
            elif edit_mode == "窓を追加":
                st.markdown(
                    "**窓追加の手順:**\n\n"
                    "1. 下の画像上で窓を追加したい**1本目の壁線をクリック**して選択（青色でハイライト）\n\n"
                    "2. **2本目の壁線をクリック**して選択（緑色でハイライト）\n\n"
                    "3. さらに窓を追加したければ手順1-2を繰り返す（2本ずつペアで選択）\n\n"
                    "4. 窓のサイズを入力:\n\n"
                    "   - 窓の高さ（mm）: 例 1200mm\n\n"
                ...
                st.markdown(
                    "**線削除の手順:**\n\n"
                    "1. 下の画像上で削除したい**壁線をクリック**して選択（赤色でハイライト）\n\n"
                    "2. さらに削除したい壁線があればクリックして追加（複数本選択可能）\n\n"
